...
* **Table Creation:** The script first ensures a clean slate by executing a `DROP TABLE IF EXISTS` followed by a `CREATE TABLE` statement.
* **Data Preparation:** Before loading, the pandas DataFrame's columns are explicitly reordered to match the SQL table schema.
* **High-Performance Data Loading:** Data is loaded using PostgreSQL's high-performance **`COPY` command** via `psycopg2`'s `copy_expert` method.
* **Post-Load Constraint Application:** Constraints (Foreign Keys, `NOT NULL`) are applied *after* the data has been copied.
* **Comprehensive Data Validation:** The script executes a series of SQL queries (row count, primary key, geospatial outliers, referential integrity) to validate the integrity of the loaded data.

---
### Alternative Load: Test/Cloud Database (Neon)

**Script:** `post_offices/scripts/upload_to_test_database.ipynb`

As an alternative to loading into the local database, this script loads the same final dataset into a cloud-hosted PostgreSQL database on **Neon DB**. This is used for testing and staging.

**For a detailed breakdown of this specific (Neon DB) loading process, see the [`Scripts README`](scripts/README.md).**
